...
const invalid = (origin: string, message: string) => json({ error: message }, 400, origin);
...
async function sendReminder(env: Env, row: ReminderRow, date: string) {
  ...
  const claim = await env.REMINDERS.prepare("UPDATE subscriptions SET last_sent_date = ?, next_retry_at = NULL WHERE endpoint = ? AND (last_sent_date IS NULL OR last_sent_date <> ?)")
  ...
  try {
    ...
    await env.REMINDERS.prepare("UPDATE subscriptions SET retry_count = 0, next_retry_at = NULL WHERE endpoint = ?").bind(row.endpoint).run();
    return true;
  } catch (error) {
    const statusCode = error instanceof webpush.WebPushError ? error.statusCode : 0;
    if (statusCode === 404 || statusCode === 410) {
      await env.REMINDERS.prepare("DELETE FROM subscriptions WHERE endpoint = ?").bind(row.endpoint).run();
    } else if (row.retry_count < 2) {
      await env.REMINDERS.prepare("UPDATE subscriptions SET last_sent_date = NULL, retry_count = retry_count + 1, next_retry_at = datetime('now', '+2 minutes') WHERE endpoint = ? AND last_sent_date = ?").bind(row.endpoint, date).run();
    } else {
      await env.REMINDERS.prepare("UPDATE subscriptions SET last_sent_date = NULL, retry_count = 0, next_retry_at = NULL WHERE endpoint = ? AND last_sent_date = ?").bind(row.endpoint, date).run();
    }
...
async function handleApi(request: Request, env: Env, path: string) {
  ...
  if (path === "/api/reminders/subscribe" && request.method === "POST") {
    try {
      ...
      if (!limit.success) return json({ error: "Bạn đã thao tác quá nhanh. Vui lòng thử lại sau một phút." }, 429, origin);
      const body = await readJson<{ subscription?: unknown; time?: unknown }>();
      if (!body || !validSubscriptionStrict(body.subscription) || !validTime(body.time)) return invalid(origin, "Subscription hoặc giờ nhắc không hợp lệ.");
      const result = await env.REMINDERS.prepare("INSERT INTO subscriptions (endpoint, subscription_json, reminder_time, updated_at) SELECT ?, ?, ?, datetime('now') WHERE EXISTS (SELECT 1 FROM subscriptions WHERE endpoint = ?) OR (SELECT COUNT(*) FROM subscriptions) < ? ON CONFLICT(endpoint) DO UPDATE SET subscription_json = excluded.subscription_json, reminder_time = excluded.reminder_time, retry_count = 0, next_retry_at = NULL, updated_at = excluded.updated_at")
  ...
  return json({ error: "Không tìm thấy API." }, 404, origin);
}
...
export default {
  ...
  async scheduled(_controller, env) {
    ...
    const { results = [] } = await env.REMINDERS.prepare("SELECT endpoint, subscription_json, reminder_time, retry_count FROM subscriptions WHERE reminder_time = ? OR (next_retry_at IS NOT NULL AND next_retry_at <= datetime('now'))").bind(now.time).all<ReminderRow>();
